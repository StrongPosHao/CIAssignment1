...
int RandomOrderingIndex(int* idx, int NumPats);
int RandomSwapOrderingIndex(int* idx, int NumPats);
int RandomNOrderingIndex(int *idx, int NumPats, int N, bool* misClassified);
...
// Ordering option 0
int FixedOrderingIndex(int *idx, int NumPats) {
...
// Ordering option 1
int RandomOrderingIndex(int *idx, int NumPats) {
  FixedOrderingIndex(idx, NumPats);
...
// x[][]=>input data, d[][]=>desired output data
  ...
  for(i=0;i<NumHN1;i++)
    ...
    bool misClassified[NumPats] = {true};
    int idx[NumPats];
    int cnt = 0;
    if (ordering == 0) {
      cnt = FixedOrderingIndex(idx, NumPats);
    } else if (ordering == 2) {
      cnt = RandomOrderingIndex(idx, NumPats);
    }

  for(;;){// Main learning loop
  // Init index array by ordering
  //  int tempNumPats = NumPats;
    if (ordering == 1) {
      cnt = RandomOrderingIndex(idx, NumPats);
    } else {
      cnt = RandomNOrderingIndex(idx, NumPats, ordering, misClassified);
    }
    MinErr=3.4e38; AveErr=0; MaxErr=-3.4e38; NumErr=0;
    for(p=0;p<cnt;p++){ // for each pattern...
      ...
      for(i=0;i<NumOPs;i++){
        ...
        if (ordering > 2) {
          misClassified[idx[p]] = ((y[i]<0.5&&d[idx[p]][i]>=0.5)||(y[i]>=0.5&&d[idx[p]][i]<0.5))?true : false;
        } 
      }
    ...
    ItCnt++;
    AveErr/=cnt;
    float PcntErr = NumErr/float(cnt) * 100.0;
